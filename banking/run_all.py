...
from src.code.banking.pyspark.steps import ingest_bronze, transform_gold, transform_silver


def main() -> int:
    """
    Run full banking pipeline: Bronze → Silver → Gold.

    Example usage:
        python -m src.code.banking.pyspark.run_all \\
            --raw-path /path/to/banking/csvs \\
            --database main \\
            --bronze demo_bronze \\
            --silver demo_silver \\
            --gold demo_gold
    """
    ap = argparse.ArgumentParser(description="Banking PySpark Pipeline")
    ...
    spark = SparkSession.builder.appName("banking-pipeline").getOrCreate()
    ...
    print("✓ Banking pipeline completed successfully!")
    spark.stop()
    return 0
