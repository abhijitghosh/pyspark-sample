...
from src.code.retail.pyspark.steps import ingest_bronze, transform_gold, transform_silver


def main() -> int:
    """
    Run full retail pipeline: Bronze → Silver → Gold.

    Example usage:
        python -m src.code.retail.pyspark.run_all \
            --raw-path /path/to/retail/csvs \
            --database main \
            --bronze demo_bronze \
            --silver demo_silver \
            --gold demo_gold
    """
    ap = argparse.ArgumentParser(description="Retail PySpark Pipeline")
